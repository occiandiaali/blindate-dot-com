...
class ArraySchema {
    ...
    static [(_a = symbols_1.$encoder, _b = symbols_1.$decoder, symbols_1.$filter)](ref, index, view) {
        return (!view ||
            ...
            view.items.has(ref['tmpItems'][index]?.[symbols_1.$changes]));
    }
    static is(type) {
        return (
        // type format: ["string"]
                            ...
                            if (previousValue !== undefined) {
                                if (setValue[symbols_1.$changes].isNew) {
                                    ...
                                    if ((obj[symbols_1.$changes].getChange(Number(key)) & spec_1.OPERATION.DELETE) === spec_1.OPERATION.DELETE) {
                                        this[symbols_1.$changes].indexedOperation(Number(key), spec_1.OPERATION.DELETE_AND_MOVE);
                                    }
                                    else {
                                        this[symbols_1.$changes].indexedOperation(Number(key), spec_1.OPERATION.MOVE);
                                    }
                                }
    ...
    $changeAt(index, value) {
        ...
        const changeTree = this[symbols_1.$changes];
        const operation = changeTree.indexes?.[index]?.op ?? spec_1.OPERATION.ADD;
    ...
    splice(start, deleteCount = this.items.length - start, ...insertItems) {
        ...
        for (let i = 0; i < insertCount; i++) {
            ...
            changeTree.indexedOperation(addIndex, spec_1.OPERATION.ADD);
